...
int tree_expr::printcode1(tree_expr *expr,char *print_string) {
    ...
    if(type==STRUCTREF) {
        ...
        if(expr->lhs!=NULL&& expr->lhs->expr_type==STRUCTREF) {
            if(expr->cpu_name!=NULL) {
                strcat(print_string,expr->cpu_name);
                GPSREF=0;
                return;
            }
            expr->lhs->printcode1(expr->lhs,print_string);
...
//if(expr->lhs->libdtype==5)strcat(print_string,".");//added M26. 5==G_P_TYPE
                tree_expr *ext1=expr;
                int dotcnt=0;
                while(ext1->expr_type==STRUCTREF) {
                    ext1=ext1->lhs;
                    dotcnt++;
                }
...
//fprintf(stderr,"//here graph type %s",expr->rhs->name);
                    ...
                    if(et1!=NULL) {
                        int mf=0,mdtype=0;
                        while(et1!=NULL) {
                            if(!strcmp(et1->name,expr->rhs->name)) {
                                mf=1;
                                mdtype=et1->t1->datatype;
            ...
            if(d1 && d1->extra_name1!=NULL ) {
                ...
                if(d1->it<6  && d1->it>=0) {
                    sprintf(temp,"((%s *)(%s.extra))->",d1->extra_name,d1->parent->name);
                    strcat(print_string,temp);
                }
                if(d1->it>=6) {
            ...
            if(flag==1 /*&& ariflag==1*/) { //now make function for field acess
                ...
                if(flag==0 && d1->arg==0 && !(d1->parent!=NULL && d1->parent->arg==1)&&d1->libdtype>=0) {
                    fprintf(stderr,"ivalid field access %s for %s datatype\n",name,libdtypenames[expr->lhs->lhs->libdtype]);
                    errflag=1;
                }
...
//strcat(print_string,temp);
    ...
    if(expr->expr_type==FUNCALL) {
        ...
        if(expr->params!=NULL) {
            ...
            while(td->next!=NULL) {
                ...
                if(td->dirrhs)td->dirrhs->printcode(1);
                strcat(print_string,",");
                td=td->next;
            }
            if(td&& td->lhs) {
    ...
    if (expr->lhs!=NULL &&expr->expr_type!=ARRREF&&expr->expr_type!=COND_OP&&expr->expr_type!=GET_TYPE) {
        if(expr->expr_type!=FUNCALL) {
            ...
            if(expr->expr_type==VAR && expr->cpu_name!=NULL) {
                strcat(print_string,expr->cpu_name);
            }
...
void tree_expr::printcode(tree_expr *expr) {
                    ...
                    if(!strcmp(ex->name,expr->rhs->name)&& strlen(ex->name)==strlen(expr->rhs->name)) {
                        if(ex->libdtype!=d1->libdtype) {
                            ...
                            if(d1->libdtype==GRAPH_TYPE && ex->libdtype!=G_P_TYPE &&ariflag==1) {
                                ...
                                expr->libdtype=ex->t1->libdatatype;    //mdofied on fe14AN MARCH24
                                flag=1;
                                break;
                            }
                        }
    ...
    if(expr->expr_type==FUNCALL) {
        ...
        if(expr->params!=NULL) {
            ...
            tree_decl_stmt *td=expr->params;
            while(td->next!=NULL) {
                if(td->lhs)fprintf(FP1,"%s ",td->lhs->name);
                if(td->dirrhs)td->dirrhs->printcode(1);
                td=td->next;
            }
