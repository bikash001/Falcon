    ...
    if(this->sttype==SINGLE_STMT) {
        ...
        if(t1&& t1->lhs &&  t1->lhs->libdtype==COLLECTION_TYPE) {
            ...
            if(this->f3) {
                ...
                elecnt=0;
                while(tt) {
                    tree_expr *ttexpr=tt->stassign->rhs;
                    while(ttexpr->lhs && ttexpr->expr_type!=VAR)ttexpr=ttexpr->lhs;
    ...
    if(this->sttype==IF_STMT) {
        ...
        if(this->f1 && this->f1->stassign && this->f1->stassign->asstype==RRMUL_ASSIGN) {
            ...
            if(Gkernel==0) {
                ...
                if(val!=5) {
                    this->expr1->findgpuvars(this->expr1);
                                        ...
                                        if(this->feb==0) {
                                            if(this->stassign!=NULL && this->stassign->rhs->expr_type==FUNCALL) {
                                                ...
                                                if(this->barrier==0)fprintf(FP1,"%s<<<temp%d/prop%d.maxThreadsPerBlock+1,prop%d.maxThreadsPerBlock>>>(",this->stassign->rhs->name,extemp+1,d2->dev_no,d2->dev_no);
                                                    ...
                                                    fprintf(FP1,"reset%s%s<<<temp%d/prop%d.maxThreadsPerBlock+1,prop%d.maxThreadsPerBlock>>>(%s ,0 );\ncudaDeviceSynchronize();\n",d2->name,ep->name,extemp+1,d2->dev_no,d2->dev_no,d2->name);
...
//else fprintf(FP1,"%s);\n",ta->rhs->name);
    ...
    if(this->sttype==DECL_STMT) {
        ...
        if(this->stdecl!=NULL && this->stdecl->dirrhs!=NULL &&this->stdecl->dirrhs->procd==1 && this->ker>=1) {
            ...
            if(strcmp(this->stdecl->dirrhs->name,"main")==0 &&strlen(this->stdecl->dirrhs->name)==strlen("main")) {
                MAINF=1;
                fgetpos(FP1,&mainfpos);
            }
            if(EXTRA_CRBRK==1) {
                    ...
                    while(t1!=NULL) {
                        if (t1->gpu==0) {
                            ...
                            if(t1->read==0) {
                                ...
                                cnt=0;
                                if(t1->nextv!=NULL)fprintf(FP1,"HGraph ");
                            }
                        }
                        t1=t1->nextv;
                    }
            ...
            if(x1->libdatatype==COLLECTION_TYPE) {
                ...
                if(Gkernel>=1) {
                    ...
                    while(t1!=NULL) {
                        ...
                        cnt++;
                        if(t1->rhs!=NULL) {
                            t1->rhs->printcode1(t1->rhs,initarr);
                            fprintf(FP1,"=%s",initarr);
                            for(int i=0; i<100; i++)initarr[0]='\0';
                        }
...
//fprintf(FP1,"%s= create_coll_device(100);\n",t1->name);t1->local=true;
        ...
        if(x1->libdatatype==-1) {
            ...
            if((x1->datatype==STRUCT_TYPE||x1->datatype==ENUM_TYPE||x1->datatype==UNION_TYPE )&&  (!(this->stdecl!=NULL && this->stdecl->dirrhs!=NULL &&this->stdecl->dirrhs->procd==1))) {
                ...
                if(this->stdecl->dirrhs!=NULL &&this->stdecl->dirrhs->procd!=1) {
                    ...
                    if(gf==2) {
                        if(Gkernel==0)fprintf(FP1,"__device__ ");
                        else fprintf(FP1,"");
                        int cnt=0;
                        tree_typedecl *tt1=this->stdecl->lhs;
                        while(tt1 && (tt1->datatype==TYPEDEF_TYPE||tt1->datatype==-1) && tt1->libdatatype==-1) {
                            tt1=tt1->next;
                        }
                        tree_typedecl *ttt1=tt1;
                        while(tt1!=NULL ) {
                            if(tt1->name!=NULL) {
                                fprintf(FP1,"%s ", tt1->name);
                            }
                            tt1=tt1->next;
                        }
                        t1=this->stdecl->dirrhs;
                        while(t1!=NULL) {
                            if (t1->gpu==1) {
                                if(cnt>0)fprintf(FP1,",");
                                if(!(t1->ctype==CARR_TYPE &&t1->next&& t1->next->assign!=NULL))t1->printcode(0);
                                cnt++;
                            }
                            t1=t1->nextv;
                        }
                    ...
                    if(gf>=1) {
                        ...
                        while(t1!=NULL) {
                            if (t1->gpu==1) {
                                if((t1->ctype==CARR_TYPE &&t1->next&& t1->next->assign!=NULL)) {


                                    tree_typedecl *Ttt1=ttt1;
                                    while(Ttt1!=NULL ) {
                                        if(Ttt1->name!=NULL) {
                                            fprintf(FP1,"%s ", Ttt1->name);
                                        }
                                        Ttt1=Ttt1->next;
                                    }
                                    fprintf(FP1,"* %s; ",t1->name);
    ...
    if(this->next && (this->sttype==DECL_STMT ||this->sttype==SBLOCK_STMT) && (this->next->sttype!=DECL_STMT||this->next->sttype!=SBLOCK_STMT) &&Gkernel==2) {
        ...
        while(t1) {
            if(t1->sttype==DECL_STMT && t1->stdecl->dirrhs!=NULL && t1->stdecl->dirrhs->procd==1)break;
            t1=t1->prev;
        }
        ...
        if(d2!=NULL) {
            if(d2->parent!=NULL) {
                if(this->barrier==1 || (this->prev &&this->prev->barrier==1)) {
                    statement *t1=this;
                    t1=t1->next;
                    while(t1 && t1->sttype==DECL_STMT && t1->next->sttype!=IF_STMT)t1=t1->next;
                    if(t1!=NULL ) {
                        t1->barrier=2;
                    }
                    for(int i=0; i<200; i++) {
            ...
            if(d2->parent && d2->parent->libdtype==COLLECTION_TYPE) {
                fprintf(FP1,"%s.size",d2->parent->name);
            }
                    ...
                    if(it1->second==d2->it) {
                        ...
                        if(this->barrier==1 || (this->prev &&this->prev->barrier==1)) {
                            if(d2->parent) {
                                ...
                                strcat(barr_after,"(( ");
                                strcat(barr_after,d2->parent->extra_name);
                                strcat(barr_after," *)(");
                                strcat(barr_after,d2->parent->name);
                                strcat(barr_after,".extra))->n");
                                strcat(barr_after,it1->first);
                            }
                            strcat(barr_after,"){");
                        }
                        ...
                        if(GLOBAL_ID_FLAG==1) {
                            d2->extra_name1=malloc(sizeof(char)*100);
                            strcpy(d2->extra_name1,"id+graph.start_point");
                        }
                        d2->iflag=1;
                        d2->extra_name=malloc(sizeof(char)*100);
                        strcpy(d2->extra_name,d2->parent->extra_name);
                        strcpy(d2->extra_name2,it1->first);
                        strcpy(d2->name,"id");
                        if(GLOBAL_ID_FLAG==1) {
                            d2->name=malloc(sizeof(char)*100);
                            strcpy(d2->name,"id+graph.start_point");
                            GLOBAL_ID_FLAG=0;
                        }
                        break;
    ...
    if(this->next && this->sttype==DECL_STMT && this->next->sttype!=DECL_STMT &&Gkernel==1) {
        Gkernel=2;
    }
