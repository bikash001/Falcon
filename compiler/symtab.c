...
alloc_extra(statement *st ) {
    ...
    if(x1->libdatatype==GRAPH_TYPE) {
        ...
        t1=st->stdecl->dirrhs;
        while(t1!=NULL) {
            if (t1->ppts!=NULL) { //printf("%s has extra properties",t1->name);
                ...
                if (t1->ppts!=NULL && t1->gpu==0) {
                    ...
                    while(epp!=NULL) {
                        if(epp->t1->libdatatype==G_P_TYPE) {
                            if(flag==0) {
                                fprintf(FP,"void read_%s_pptysize(HGraph &%s){\n",t1->name,t1->name);
                                flag=1;
                                exread=1;
                            }
...
int tree_decl_stmt::printcode(int f) {
    ...
    if(x1->def==true) {
        ...
        if(x1->datatype==ENUM_TYPE) {
            ...
            while(head!=NULL) { // printf("%s,\n",head->name);
                if(head->idrhs!=NULL) {
                    head=head->next; /*error, need to fix*/
                } else {
                    printf("%s\n", "ERROR @ symbtab.c, line-777");
                    head = NULL;
                }
            }
            printf("%s\n", "syntab-776");
        ...
        while(head!=NULL) {
            if(head->lhs->compoundtype==1) {
                if(head->lhs->datatype==STRUCTREC_TYPE/*&& ( (strcmp(this->lhs->vname,head->lhs->vname)!=0)||(strlen(this->lhs->vname)!=strlen(head->lhs->vname)))*/)
                    head->printcode(0);
                head=head->next;
                continue;
            }
            ...
            while(t1!=NULL)if(t1->name!=NULL) {
                    ...
                    else fprintf(FP1,"/*%d*/%s ", t1->libdatatype,t1->name);
                    t1=t1->next;
                }
            if(head->dirrhs!=NULL) {
    ...
    dir_decl *t1=this->dirrhs;
    while(t1!=NULL) {
        if (t1->gpu==1 && t1->parent==NULL)gf=1;
        if(t1->gpu==0&& t1->parent==NULL)hf=1;
        if(hf&&gf) break;
        t1=t1->nextv;
    }
    if(gf==1) {
        if(Gkernel==0)fprintf(FP1,"__device__ ");
        else fprintf(FP1,"");//fprintf(FP1,"__device__ ");
        int cnt=0;
        tree_typedecl *tt1=this->lhs;
        while(tt1 && (tt1->datatype==TYPEDEF_TYPE||tt1->datatype==-1) && tt1->libdatatype==-1) {
            tt1=tt1->next;
        ...
        while(tt1!=NULL ) {
            if(tt1->datatype==STRUCTREC_TYPE) {
                fprintf(FP1, "int ");
                break;
            }
            if(tt1->name!=NULL) {
    ...
    if(gf==1) {
        if(Gkernel==0)fprintf(FP1,"__device__ int ");
        else fprintf(FP1,"int ");//fprintf(FP1,"__device__ ");
        t1=this->dirrhs;
        while(t1!=NULL) {
            if (t1->gpu==1) {
                if(cnt>0)fprintf(FP1,",");
    ...
    cnt=0;
    if(hf==1) {
        fprintf(FP1,"int ");//fprintf(FP1,"__device__ ");
        t1=this->dirrhs;
        while(t1!=NULL) {
            if (t1->gpu==0) {
                if(cnt>0)fprintf(FP1,",");
...
void dir_decl::printcodearray(int ref,char *print_string) {
    ...
    if(t1->ptrcnt>0) {
        if(t1->ptrstring!=NULL)strcat(print_string,t1->ptrstring);
    };
    if(t1->name!=NULL) {
...
void dir_decl::printcode(int ref) {
    ...
    if(t1->ptrcnt>0) {
        if(t1->ptrstring!=NULL)fprintf(FP1," %s ",t1->ptrstring);
    };
    if(t1->name!=NULL) {
        if( t1->gpu) {
...
tree_typedecl *symtable::findtypedefsymbol(char *name) {
    ...
    tree_typedecl *t1;
    for (std::map<char *,tree_typedecl *>::iterator it=this->newtypes.begin(); it!=this->newtypes.end(); ++it) {
        if(!strcmp(it->first,name)&&(strlen(it->first)==strlen(name))) {
            t1=it->second;
            flag=1;
            break;
        }
...
dir_decl *libsymtable::findsymbol(char *name) {
    ...
    for (I = entries.begin(); I != entries.end(); I++) {
        ...
        for (J =e->vars.begin(); J != e->vars.end(); J++) {
            variable= *J;
            char* c = variable->name;
            if(! strcmp(c,name)&&(strlen(c)==strlen(name))) {
                return  variable;
            }
        }
    }
    ...
    while(head!=NULL) {
        for (I = entries.begin(); I != entries.end(); I++) {
            ...
            for (J =e->vars.begin(); J != e->vars.end(); J++) {
                variable= *J;
                char* c = variable->name;
                if(! strcmp(c,head->name)&&(strlen(c)==strlen(head->name))) {
                    return  e;
                }
            }
    ...
    if(type1->libdatatype!=-1) {
        if(type1->libdatatype==E_P_TYPE ||type1->libdatatype==P_P_TYPE || type1->libdatatype==G_P_TYPE) {
            ...
            if(d2!=NULL) {
                ...
                if(this->libsym==NULL)  {
                    this->libsym=new libsymtable();
                    this->libsym->gparent=this;
                }
...
int check_gpu(statement *t1) {
    ...
    rs=t1->stassign->rhs;
    if(rs!=NULL&& check_gpu_expr(rs)==1) {
        if(ret==1)ret=3;
        else ret=2;
    }
    return ret;
        ...
        if(t1->expr1->lhs->lhs->libdtype>=0) {
            ...
            if((t1->expr1->lhs->lhs->libdtype==0|| t1->expr1->lhs->libdtype==3)&& t1->expr1->rhs->lhs->libdtype==t1->expr1->lhs->lhs->libdtype) {
                if(t1->expr1->lhs->lhs->clone==0) {
                    ...
                    fprintf(FP1,"%s.copytoCPU(%s);",arr2,arr1);
                }
            }
            tree_expr *te1=t1->expr1->lhs;
            while(te1->lhs!=NULL)te1=te1->lhs;
        ...
        if(t1->stassign->lhs->lhs->libdtype>=0) {
            if((t1->stassign->lhs->libdtype==0|| t1->stassign->lhs->libdtype==3)&& t1->stassign->rhs->libdtype==t1->stassign->lhs->libdtype) {
                if(t1->stassign->lhs->lhs->clone==0) {
                    ...
                    fprintf(FP1,"%s.copytoGPU(%s);",arr2,arr1);
                }
