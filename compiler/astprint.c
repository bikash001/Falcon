...
statement::print() {
    ...
    if(this->sttype==ASSIGN_STMT) {
        if(this->stassign->rhs==NULL&&this->sttype==ASSIGN_STMT) {
            if(this->stassign->lhs!=NULL/* && this->stassign->lhs->name!=NULL*/) {
                this->stassign->lhs->print(this->stassign->lhs);
            }
        }
        ...
        if(d2 && d2->libdtype==COLLECTION_TYPE) {
            if(it==5) {
                ...
                if(d2->tp1 && d2->tp1->next==NULL) {
                    if(this->expr1) {
                        this->expr1->lhs->libdtype=-1;;
                        this->expr1->lhs->dtype=d2->tp1->datatype;
                    }
                    ...
                    if(this->expr1) {
                        ...
                        this->expr1->lhs->libdtype=d2->tp1->next->libdatatype;
                        this->expr1->lhs->dtype=d2->tp1->next->datatype;
                    }
                }
                else {
                    if(this->expr1) {
                        this->expr1->lhs->libdtype=COLLECTION_TYPE;
                        ((dir_decl *)this->expr1->lhs)->tp1=d2->tp1->next;
                    }
        ...
        if(d2 && d2->libdtype==SET_TYPE ) {
            if(it==5) {
                ...
                if(d2->tp1 && d2->tp1->next==NULL) {
                    ...
                    if(this->expr1) {
                        this->expr1->lhs->libdtype=SET_TYPE;
                        ((dir_decl *)this->expr1->lhs)->tp1=d2->tp1->next;
                    }
                }
            }
    ...
    if(this->sttype==FOR_STMT) {
        ...
        if(this->f2 && this->f2->stassign!=NULL) {
            this->f2->stassign->print1(0);
        }
        ...
        if(x1->libdatatype==-1) {
            ...
            if(x1->datatype==STRUCT_TYPE||x1->datatype==ENUM_TYPE||x1->datatype==UNION_TYPE) {
                ...
                if(this->stdecl->dirrhs!=NULL &&this->stdecl->dirrhs->procd!=1) {
                    ...
                    while(t1 && (t1->datatype==TYPEDEF_TYPE||t1->datatype==-1) && t1->libdatatype==-1) {
                        t1=t1->next;
                    }
                    while(t1!=NULL ) {
                        if(t1->name!=NULL) {
...
//if(ERRPRINT)printf("error LINE:-%d  graph extra property %s does not exist \n",FALCLINENO,expr->rhs->name);
            ...
            if(d1 && d1->libstable!=NULL) {
                ...
                if(d1->libdtype==GRAPH_TYPE ) {
                    if(strstr(expr->rhs->name,"read")!=NULL) {
                        d1->read=1;
                ...
                if(flag==1 /*&& ariflag==1*/) {
                    if(expr->lhs->lhs->libdtype==GRAPH_TYPE) {
                        if(((i==1)||(i==3)) && expr->rhs->expr_type!=ARRREF &&ariflag==1) { //1=points,3=edges
                            if (ERRPRINT) printf("%s\n", "error @ astprint.c - 653");
                            errflag=1;
                        }
                        else if(expr->rhs->expr_type==ARRREF&&((i==0)||(i==2)|| (i==4)|| (i==5))/*&&ariflag==1*/) {
                            if (ERRPRINT) printf("%s\n", "error @ astprint.c - 657");
                            errflag=1;
                            return;
                        }
                        else if((i==1) /*&& expr->rhs->expr_type==ARRREF*/) {
                            expr->libdtype=POINT_TYPE; /*printf("point type\n");*/expr->parent=d1;
                        }
                    ...
                    if(expr->lhs->lhs->libdtype==POINT_TYPE) {
                        ...
                        if((i==4) && expr->rhs->expr_type==ARRREF) {
                            expr->libdtype=POINT_TYPE;
                        }
                        if((i==5) && expr->rhs->expr_type==ARRREF) {
                            expr->libdtype=POINT_TYPE;
                        }
                        if((i==6) && expr->rhs->expr_type==ARRREF) {
                            expr->libdtype=POINT_TYPE;
                        }
                    }
                    if(expr->lhs->lhs->libdtype==EDGE_TYPE) {
                        ...
                        if((i==0) && expr->rhs->expr_type==VAR) {
                            /*printf("edGE TYPE %d\n",i);*/expr->libdtype=POINT_TYPE;
                        }
                        if((i==1) && expr->rhs->expr_type==VAR) {
                            /*printf("edGE TYPE %d\n",i); */ expr->libdtype=POINT_TYPE;
                        }
                    }
                ...
                if(flag==0) {
                    ...
                    else if(d1->parent!=NULL) {
                        ex=d1->parent->ppts;/*fprintf(stderr,"parent=%d %s %s ",d1->parent->arg,d1->name,d1->parent->name);*/
                    }
                        ...
                        if(!strcmp(ex->name,expr->rhs->name)&& strlen(ex->name)==strlen(expr->rhs->name)) {
                            if(ex->libdtype!=d1->libdtype) {
                                ...
                                if(d1->libdtype==GRAPH_TYPE && ex->libdtype!=G_P_TYPE &&ariflag==1) {
                                    ...
                                    expr->libdtype=ex->t1->libdatatype;
                            ...
                            if(ex->t1->datatype!=-1&& expr->rhs->expr_type==VAR) {
                                expr->libdtype=ex->t1->libdatatype;
                                expr->dtype=ex->t1->datatype;
                            }
                            if(ex->t1->libdatatype==-1&& expr->rhs->expr_type==VAR) {
                                expr->libdtype=ex->t1->libdatatype;
                                expr->dtype=ex->t1->datatype;
                            }
                            flag=1;
                ...
                if(flag==0 &&d1->libdtype!=-1) {
                    ...
                    if(d1->libdtype==GRAPH_TYPE)ex=d1->ppts;
                        ...
                        ex=d1->parent->ppts;/*fprintf(stderr,"Parent=%d %s ",d1->parent->arg,d1->parent->name);*/
                    }
                    ...
                    ex->t1=t1;
                    if(d1->libdtype==GRAPH_TYPE &&d1->arg==1) {
                        ex->next=d1->ppts;
                        d1->ppts=ex;
                    }
                    else if(d1->parent!=NULL) {
                        ex->next=d1->parent->ppts;
                        d1->parent->ppts=ex;
                    }
                    else if(d1->arg==1) {
                        ex->next=d1->ppts;
                ...
                expr->lhs->print(expr->lhs);
                expr->libdtype=expr->lhs->libdtype;/*printf("printing lhs first %d \n",expr->lhs->libdtype);*/if(expr->lhs->libdtype==-2) {
                    if(ERRPRINT)
                ...
                if(expr->lhs && expr->lhs->parent!=NULL) {
                    ...
                    while(ex!=NULL&& flag==0) {
                        if(!strcmp(ex->name,expr->rhs->name)&& (strlen(ex->name)==strlen(expr->rhs->name))) {
                            flag=1;
                            if(ex->t1->datatype!=-1) {
                                expr->libdtype=-1;//THIS MAY NOT BE SUFFICIENT FEB 14 FOUND OUT
                                expr->dtype=ex->t1->datatype;
                            }
                            break;
                        }
    ...
    if(expr->expr_type==VAR ) {
        if(expr->lhs!=NULL && expr->lhs->libdtype>=0&&expr->libdtype<0&&sref==0&&/*ariflag==0&&*/expr->rhs==NULL) {
    ...
    if(expr->expr_type==FUNCALL) {
        ...
        if(expr->name!=NULL) {
            ...
            if(expr->name!=NULL) {
                char temp[20];
                for(int i=0; i<11; i++) {
                    if(!strcmp(libfunctions[i],expr->name) && strlen(libfunctions[i])==strlen(expr->name)) {
                        ind=i;
                        libf=1;
                        break;
                    }
                            ...
                            if((tt->dirrhs && tt->dirrhs->libdtype!=-1) ||(ta->rhs  && ta->rhs->lhs&& ta->rhs->lhs->forit==1)) {
                                if(ta->rhs && ta->rhs->lhs && ta->rhs->lhs->libdtype==-1 && ta->rhs->lhs->forit!=1) {
                                    ...
                                    while(et!=NULL) {
                                        ...
                                        while(et1!=NULL) {
                                            if(!strcmp(et1->name,et->name)&&(strlen(et1->name)==strlen(et->name)))break;
                                            et1=et1->next;
                                        }
                                        if(et1==NULL) {
...
//printf("NULL\n");
    ...
    if (expr->lhs!=NULL &&expr->expr_type!=ARRREF&&expr->expr_type!=COND_OP&&expr->expr_type!=GET_TYPE) {
        if(expr->expr_type!=FUNCALL) {
...
void assign_stmt::print1(bool semi) {
    ...
    if(ariflag==0) {
        ...
        if(this->lhs && this->lhs->libdtype>=0 ) {
            ...
            if(this->lhs) {
                d1=this->lhs->parent;
                while(d1&&d1->parent!=NULL) d1=d1->parent;
            }
            if(this->rhs) {
                d2=this->rhs->lhs;
                while(d2 &&d2->parent!=NULL) d2=d2->parent;
            }
        }
    }
